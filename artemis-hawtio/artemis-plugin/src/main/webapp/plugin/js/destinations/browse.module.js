...
var Artemis;
(function (Artemis) {
    Artemis.log.info("loading browse queue");
    Artemis.browseQueueModule = angular
    .module('artemis-browse-queue', [])
    .component('artemisBrowseQueue', {
        template:
            `<h1>Browse Queue
            ...
            <div class="form-group">
                <label>filter:
                    <input type="input" ng-model="$ctrl.filter">
                </label>
                <label>fetch size:
                    <input type="input" ng-model="$ctrl.resultsSize">
                </label
                <label>page:
                    <input type="input" ng-model="$ctrl.currentResultsPage">
                </label>
            <button class="btn btn-primary" ng-click="$ctrl.refresh()">Refresh</button>
            </div>
            <div ng-show="$ctrl.isLoading">
                <p><span class="spinner spinner-lg spinner-inline"></span> Loading Messages.....</p>
            </div>
            ...
            <div class="table-view artemis-browse-main" ng-show="!$ctrl.showMessageDetails">
                ...
                <pf-table-view config="$ctrl.tableConfig"
                    ...
                    menu-actions="$ctrl.tableMenuActions">
                </pf-table-view>
            ...
            <script type="text/ng-template" id="browse-instructions.html">
              <div>
                <p>
                  This page allows you to browse messages on a queue in Artemis. The <code>fetch size</code> controls how
                  many messages are returned from the Artemis server and <code>page</code> controls the start point in
                  the queue of where to start browsing messages. Once loaded the messages can be sorted and searched using
                  the table controls.
                </p>
                <p>
                Note that setting the <code>fetch size</code> too high could take a long time and also cause the browser
                to run out of memory
                </p>
                <p>
                  Messages can also be filtered at the broker using the <code>filter</code>: see <a href="https://activemq.apache.org/components/artemis/documentation/latest/filter-expressions.html" target="_blank">Filter Expressions</a>
                </p>
              </div>
            </script>
        `,
    ...
    function BrowseQueueController($scope, workspace, jolokia, localStorage, artemisMessage, $location, $timeout, $filter) {
        Artemis.log.info("loaded browse " + Artemis.browseQueueModule);
        var ctrl = this;
        ctrl.resultsSize = 100;
        ctrl.isLoading = false;
        ctrl.currentResultsPage = 1;
        ctrl.actionText = '';
        ...
        var deleteConfig = {
            ...
            actionFn: openDeleteDialog,
            isDisabled: true
        };
        var moveConfig = {
            name: 'Move',
            title: 'Delete selected messages',
            actionFn: openMoveDialog,
            isDisabled: true
        };
        var retryConfig = {
            name: 'Retry',
            title: 'retry sending messages to original queue',
            actionFn: openMoveDialog,
            isDisabled: true
        };
        ...
        if (objName) {
            ...
            if (queueName == artemisDLQ || queueName == artemisExpiryQueue) {
                actionsConfig.primaryActions.push(retryConfig);
            }
        }
        ctrl.toolbarConfig = {
            filterConfig: {
                fields: [
                    ...
                    },
                    {
                        id: 'redelivered',
                        title: 'Redelivered',
                        placeholder: 'filter by redelivered',
                        filterType: 'select',
                        filterValues: ['true', 'false']
                    }
                ],
                resultsCount: ctrl.messages.length,
                totalCount: ctrl.allMessages.length,
                appliedFilters: [],
        ...
        BrowseQueueController.prototype.$onInit = function () {
            this.loadData();
        };
        BrowseQueueController.prototype.loadData = function () {
            Artemis.log.info("loading table2")
            loadTable();
        };
        ...
        Artemis.log.info("loaded browse 5" + Artemis.browseQueueModule);
        ctrl.currentMessage;
        ...
        var ignoreColumns = ["PropertiesText", "bodyText", "text", "headers", "properties", "textMode"];
        ...
        function openMoveDialog(action, item) {
            ...
            Artemis.log.info(ctrl.actionText);
        ...
        function handleCheckBoxChange (item) {
            var selectedItems = $filter('filter')(ctrl.messages, {selected: true});
            Artemis.log.info("sel " + selectedItems.length);
            if (selectedItems.length == 0) {
                deleteConfig.isDisabled = true;
                moveConfig.isDisabled = true;
                retryConfig.isDisabled = true;
                return;
            }
            moveConfig.isDisabled = false;
            deleteConfig.isDisabled = false;
            retryConfig.isDisabled = false;
        }

        function openDeleteDialog(action, item) {
            var selectedItems = $filter('filter')(ctrl.messages, {selected: true});
            if(!selectedItems) {
                selectedItems = [];
                return;
        ...
        function openRetryDialog(action, item) {
            var selectedItems = $filter('filter')(ctrl.messages, {selected: true});
            if(!selectedItems) {
                return;
            }
        ...
        function populateTable(response) {
            ...
            if (ctrl.queueNames.length === 0) {
                var queueNames = Artemis.getSelectionQueuesFolder(workspace);
        ...
        */
        ...
        function createProperties(message) {
            ...
            angular.forEach(message, function (value, key) {
                if (!_.some(ignoreColumns, function (k) { return k === key; }) && _.some(flattenColumns, function (k) { return k === key; })) {
                    Artemis.log.info("key=" + key + " value=" + value);
                    angular.forEach(value, function (v2, k2) {
                    Artemis.log.info("key=" + k2 + " value=" + v2);
        ...
        function loadTable() {
            ...
            if (objName) {
                ...
                jolokia.request({ type: 'exec', mbean: objName, operation: 'countMessages()'}, Core.onSuccess(function(response) { ctrl.toolbarConfig.filterConfig.resultsCount = response.value; }));
                jolokia.request({ type: 'exec', mbean: objName, operation: 'browse(int, int, java.lang.String)', arguments: [ctrl.currentResultsPage, ctrl.resultsSize, ctrl.filter] }, Core.onSuccess(populateTable));
            }
        }
        ...
        function operationSuccess() {
            ...
            ctrl.toolbarConfig.filterConfig.selectedCount = 0;
        ...
        function applyFilters(filters) {
            Artemis.log.info("filters " + filters);
            ctrl.messages = [];
        ...
        var matchesFilters = function (message, filters) {
            ...
            filters.forEach(function(filter) {
                ...
                Artemis.log.info("filter " + filter.id);
      ...
      function buildSearchConditions(filterText) {
       ...
       Artemis.log.info("loaded browse ref 3" + Artemis.browseQueueModule);
   }
    BrowseQueueController.$inject = ['$scope', 'workspace', 'jolokia', 'localStorage', 'artemisMessage', '$location', '$timeout', '$filter'];
